...
`;
const PANEL = `<div id="model-evaluation" style="margin-top:18px;background:#121a2f;border:1px solid #293756;border-radius:12px;padding:16px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap"><b>BƯỚC 10 — KIỂM ĐỊNH MÔ HÌNH</b><span id="model-status" style="color:#94a3c5">Đang kiểm tra...</span></div><div id="model-grid" style="display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:8px;margin-top:12px"></div><div id="model-decision" style="margin-top:10px;padding:10px 12px;border-radius:8px;background:#0a1122;border:1px solid #293756;color:#d7e0f2;line-height:1.55">Đang tải...</div><div style="margin-top:8px;color:#94a3c5;font-size:12px">Mô hình thử nghiệm là bản shadow dùng để kiểm định trên chính các lệnh DEMO đã đóng. Chưa đủ bằng chứng thì không thay mô hình đang dùng.</div></div><style>@media(max-width:900px){#model-grid{grid-template-columns:repeat(2,minmax(150px,1fr))!important}}@media(max-width:600px){#model-grid{grid-template-columns:1fr!important}}</style><script>(async()=>{const f=n=>Number.isFinite(Number(n))?Number(n).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}):'--';const pct=n=>Number.isFinite(Number(n))?Number(n).toFixed(2)+'%':'--';const card=(title,v)=>'<div style="background:#0d1426;border-radius:8px;padding:10px">'+title+'<b style="display:block;margin-top:4px">'+v+'</b></div>';const modelName=v=>v==='rule-v1'?'rule-v1':v==='rule-v2-shadow'?'rule-v2-shadow':v;const decisionText=v=>v==='PROMOTE_READY'?'SẴN SÀNG THAY THẾ':v==='KEEP_CURRENT'?'GIỮ MÔ HÌNH HIỆN TẠI':v;async function check(){try{const r=await fetch('/api/paper/model-evaluation?ts='+Date.now(),{cache:'no-store'}),d=await r.json();if(!r.ok||d.ok===false)throw Error(d.error||('HTTP '+r.status));const a=d.active,c=d.candidate;document.getElementById('model-status').textContent='✓ BỘ KIỂM ĐỊNH MÔ HÌNH ĐANG CHẠY';document.getElementById('model-status').style.color='#55dc92';document.getElementById('model-grid').innerHTML=card('Mô hình đang dùng',modelName(a.model))+card('Mô hình thử nghiệm',modelName(c.model))+card('Mẫu kiểm định',d.evaluatedTrades+'/'+d.minimumTrades)+card('Quyết định',decisionText(d.decision))+card('P&L mô hình đang dùng',f(a.pnl))+card('P&L mô hình thử nghiệm',f(c.pnl))+card('R trung bình mô hình đang dùng',Number(a.avgR).toFixed(2)+'R')+card('R trung bình mô hình thử nghiệm',Number(c.avgR).toFixed(2)+'R')+card('Hệ số lợi nhuận mô hình đang dùng',a.profitFactor==null?'∞':Number(a.profitFactor).toFixed(2))+card('Hệ số lợi nhuận mô hình thử nghiệm',c.profitFactor==null?'∞':Number(c.profitFactor).toFixed(2))+card('Sụt giảm tối đa mô hình đang dùng',f(a.maxDrawdown))+card('Sụt giảm tối đa mô hình thử nghiệm',f(c.maxDrawdown))+card('Độ chính xác mô hình đang dùng',pct(a.forecastAccuracy))+card('Độ chính xác mô hình thử nghiệm',pct(c.forecastAccuracy))+card('Phạm vi áp dụng mô hình thử nghiệm',pct(c.coverage));document.getElementById('model-decision').textContent=(d.decision==='PROMOTE_READY'?'✓ Mô hình thử nghiệm đạt điều kiện thay thế nhưng chưa tự áp dụng.':'⏸ Giữ mô hình hiện tại: ')+d.reason}catch(e){document.getElementById('model-status').textContent='✕ LỖI BỘ KIỂM ĐỊNH MÔ HÌNH';document.getElementById('model-status').style.color='#ff7181';document.getElementById('model-decision').textContent='Lỗi: '+e.message}}check();setInterval(check,10000)})();</script>`;
