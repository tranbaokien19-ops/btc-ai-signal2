...
function validateTarget(side, entry, stopLoss, tp1, tp2) {
  ...
  if ((side === 'LONG' && stopLoss >= entry) || (side === 'SHORT' && stopLoss <= entry)) {
    return 'Stop Loss không hợp lệ cho hướng lệnh';
  }
  if (tp1 != null && (!(tp1 > 0) || (side === 'LONG' ? tp1 <= entry : tp1 >= entry))) {
    return 'TP1 không hợp lệ cho hướng lệnh';
  }
  if (tp2 != null && (!(tp2 > 0) || (side === 'LONG' ? tp2 <= entry : tp2 >= entry))) {
    return 'TP2 không hợp lệ cho hướng lệnh';
  }
  if (tp1 != null && tp2 != null && (side === 'LONG' ? tp2 < tp1 : tp2 > tp1)) {
    return 'TP2 phải nằm xa Entry hơn TP1';
  }
  return null;
}
...
function snapshotPosition(p, price, at = Date.now()) {
  ...
  return {
    ...
    mfePriceMove: mfe,
    maePriceMove: mae,
    mfeR: p.riskAmount > 0 ? mfe * p.quantity / p.riskAmount : null,
    maeR: p.riskAmount > 0 ? mae * p.quantity / p.riskAmount : null,
    durationSec: Math.floor(elapsed / 1000),
    lastMarkedAt: new Date(at).toISOString()
  };
}
...
function closedStats(s) {
  ...
  let equity = s.initialCapital;
  let maxDrawdown = 0;
  for (const t of closed) {
    equity += num(t.realizedPnl, 0) || 0;
  ...
  return {
    ...
    avgR,
    maxDrawdown,
    maxDrawdownPct: s.initialCapital > 0 ? maxDrawdown / s.initialCapital * 100 : 0
  };
}
...
export class PaperTrading extends DurableObject {
  ...
  async markAndMaybeClose(s, price, source = 'HTTP') {
    ...
    if (!(hitSL || hitTP2 || hitTP1)) {
      s.position = p;
      s.trades = s.trades.map(t => t.id === p.id ? { ...t, ...p } : t);
      return { state: s, closed: null };
    }
    ...
    const closed = {
      ...
      realizedR: p.unrealizedR,
      durationSec: Math.floor((Date.now() - new Date(p.openedAt).getTime()) / 1000)
    };
    s.capital += closed.realizedPnl;
    s.realizedPnl += closed.realizedPnl;
    s.position = null;
    s.trades = s.trades.map(t => t.id === closed.id ? closed : t);
  ...
  async alarm() {
    ...
    try {
      const r = await fetch(`${MARKET_API}/products/BTC-USD/ticker`, {
        headers: { accept: 'application/json', 'user-agent': 'btc-ai-signal2-paper/1.0' }
      });
      ...
      s.engineError = String(e?.message || e);
      s.updatedAt = nowIso();
      await this.ctx.storage.put('state', s);
      await this.ctx.storage.setAlarm(Date.now() + MONITOR_MS);
    }
  }
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/status' && method === 'GET') {
      ...
      return Response.json({
        ...
        ...s,
        position,
        equity: s.capital + (position?.unrealizedPnl || 0),
        openTrades: s.trades.filter(t => t.status === 'OPEN').length,
        stats: closedStats(s),
    ...
    if (url.pathname === '/reset' && method === 'POST') {
      ...
      return Response.json({ ok: true, ...s, stats: closedStats(s) });
    }
    ...
    if (url.pathname === '/open' && method === 'POST') {
      ...
      const stopLoss = num(b.stopLoss);
      const tp1 = num(b.takeProfit1, null);
      const tp2 = num(b.takeProfit2, null);
      const validation = validateTarget(side, entry, stopLoss, tp1, tp2);
      if (validation) return Response.json({ ok: false, error: validation }, { status: 400 });

      const leverage = Math.max(1, Math.min(MAX_LEVERAGE, num(b.leverage, DEFAULT_LEVERAGE)));
      const riskPct = Math.max(MIN_RISK_PCT, Math.min(MAX_RISK_PCT, num(b.riskPct, DEFAULT_RISK_PCT)));
      const riskAmount = s.capital * riskPct / 100;
      const stopPct = Math.abs(entry - stopLoss) / entry;
      const notional = Math.min(s.capital * leverage, riskAmount / stopPct);
      ...
      s.position = {
        ...
        timeframe: b.timeframe || 'M5/M15/M30/H1/H4',
        signal: b.signal || side,
        entryReason: b.entryReason || null
      };
      s.engineError = null;
      s.trades = [...s.trades, s.position].slice(-MAX_TRADES);
      await this.saveState(s);
      await this.ctx.storage.setAlarm(Date.now() + MONITOR_MS);
    ...
    if (url.pathname === '/tick' && method === 'POST') {
      ...
      if (!(price > 0)) return Response.json({ ok: false, error: 'Thiếu price hợp lệ' }, { status: 400 });
      const result = await this.markAndMaybeClose(s, price, b.source || 'HTTP');
      s = result.state;
      await this.saveState(s);
      if (s.position) await this.ctx.storage.setAlarm(Date.now() + MONITOR_MS);
    ...
    if (url.pathname === '/close' && method === 'POST') {
      ...
      const b = await request.json().catch(() => ({}));
      const price = num(b.price, s.lastPrice);
      if (!(price > 0)) return Response.json({ ok: false, error: 'Thiếu price hợp lệ để đóng lệnh' }, { status: 400 });
      const p = snapshotPosition(s.position, price);
      const closed = {
        ...p,
        status: 'CLOSED',
        exit: price,
        exitReason: String(b.reason || 'MANUAL'),
        closeSource: 'HTTP',
        closedAt: nowIso(),
        realizedPnl: p.unrealizedPnl,
        realizedR: p.unrealizedR,
