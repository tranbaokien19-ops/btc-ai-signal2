...
function planFromForecast(side, price, f24, m5, m15) {
  if (!f24 || !['LONG','SHORT'].includes(side)) return null;
  const ema20s = [m5?.ema20,m15?.ema20].map(Number).filter(Number.isFinite);
  if (side === 'SHORT') {
    const structureLow = Number(f24.resistance), structureHigh = Number(f24.resistanceHigh);
    const ema = ema20s.length ? Math.max(...ema20s) : price;
    let low = Math.max(structureLow, ema);
    let high = Math.max(structureHigh, ema);
    ...
    const risk = Math.max(price*.0035, (high-low)*.75, entry*.0035);
    const stop = high + risk*.55;
    const t = adaptiveTargets('SHORT', entry, stop-entry, f24);
  ...
  const structureHigh = Number(f24.support), structureLow = Number(f24.supportDeep);
  const ema = ema20s.length ? Math.min(...ema20s) : price;
  ...
  const entry = (low + high) / 2;
  const risk = Math.max(price*.0035, (high-low)*.75, entry*.0035);
  const stop = low - risk*.55;
  const t = adaptiveTargets('LONG', entry, entry-stop, f24);
...
function adaptiveTargets(side, entry, stopDistance, f24) {
  ...
  if (side === 'LONG') {
    const r1 = Number(f24.resistance); const r2 = Number(f24.resistanceHigh); const ext = Number(f24.range24hHigh);
  ...
  const r1 = Number(f24.support); const r2 = Number(f24.supportDeep); const ext = Number(f24.range24hLow);
