...
function buildScenarioSet(price, f24, m5, m15) {
  if (!f24 || !(price > 0)) return null;
  const ema20s = [m5?.ema20,m15?.ema20].map(Number).filter(Number.isFinite);
  const emaLow = ema20s.length ? Math.min(...ema20s) : Number(f24.support);
  const emaHigh = ema20s.length ? Math.max(...ema20s) : Number(f24.resistance);
  const support = Number(f24.support), deep = Number(f24.supportDeep), resistance = Number(f24.resistance), high = Number(f24.resistanceHigh);
  const zone = (low, highZone) => ({ low:Math.min(low,highZone), high:Math.max(low,highZone) });
  const longAZone = zone(Math.max(deep,Math.min(support,emaLow)*0.998),Math.min(support,Math.max(deep,emaLow)*1.002));
  const longBZone = zone(Math.max(0,deep*0.998),Math.min(support*0.998,deep*1.01));
  const shortAZone = zone(Math.max(resistance,Math.min(high,emaHigh)*0.998),Math.min(high,Math.max(resistance,emaHigh)*1.002));
  const shortBZone = zone(Math.max(resistance*1.002,high*0.998),high*1.002);
  const make = (side,label,z,mult=.75) => { let entry=(z.low+z.high)/2; if(!(entry>0))entry=price; const width=Math.max(price*.0015,z.high-z.low), risk=Math.max(price*.0035,width*mult,entry*.0035); const stop=side==='LONG'?z.low-risk*.55:z.high+risk*.55; const t=adaptiveTargets(side,entry,Math.abs(entry-stop),f24); return {id:label,side,entryLow:z.low,entryHigh:z.high,plannedEntry:entry,plannedStop:stop,plannedTp1:t.tp1,plannedTp2:t.tp2,plannedTp3:t.tp3,plannedRR:t.rr,condition:side==='LONG'?'chờ giá chạm vùng hỗ trợ + reclaim/xác nhận M5/M15':'chờ giá chạm vùng kháng cự + reject/xác nhận M5/M15'}; };
  const scenarios={longA:make('LONG','LONG_A',longAZone),longB:make('LONG','LONG_B',longBZone,1),shortA:make('SHORT','SHORT_A',shortAZone),shortB:make('SHORT','SHORT_B',shortBZone,1)};
  scenarios.active='WAIT — chưa kích hoạt kịch bản nào; phải chờ giá vào đúng vùng và xác nhận';
  return scenarios;
}
...
function buildAutoSignal(data, price) {
  ...
  const weights={M5:.10,M15:.15,M30:.20,H1:.25,H4:.30}; const scores=Object.fromEntries(required.map(tf=>[tf,timeframeScore(byTf[tf])])); const score=Math.round(Object.entries(weights).reduce((sum,[tf,w])=>sum+scores[tf]*w,0)); const confidence=Math.round(Math.abs(score-50)*2); const bullish=required.filter(tf=>scores[tf]>=60).length; const bearish=required.filter(tf=>scores[tf]<=40).length; const highBull=scores.H1>=60&&scores.H4>=60; const highBear=scores.H1<=40&&scores.H4<=40; const f24=forecast24h(data,price); const m5=byTf.M5,m15=byTf.M15; const scenarios=buildScenarioSet(price,f24,m5,m15); const m5Prev=previousClose(m5),m15Prev=previousClose(m15); const priceToSupport=f24.support>0?Math.max(0,(price-f24.support)/price):1; const priceToResistance=f24.resistance>0?Math.max(0,(f24.resistance-price)/price):1;
...
async function autoDemo(env){
  ...
  // Always refresh the 24H analysis for the dashboard, even when a DEMO order is already open.
  // Timeframe data is cached in timeframes.js, so this does not hammer Coinbase.
  const [tf,price]=await Promise.all([getFiveTimeframes(),getTickerPrice()]);
  const plan=buildAutoSignal(tf,price);

  if(status.position){
    return{ok:true,signal:plan.signal,score:plan.score,confidence:plan.confidence,scores:plan.scores,qualityGate:plan.qualityGate,entryMode:plan.entryMode,learningEligible:plan.learningEligible,forecast24h:plan.forecast24h,plan:plan.plan,scenarios:plan.scenarios,position:status.position,reason:'Đang có DEMO ORDER — tiếp tục theo dõi và đối chiếu forecast 24H'};
  }

  const historyRes=await paper.fetch('https://paper/history?limit=1');
  const history=await historyRes.json();
  const last=history?.trades?.at(-1);
  if(last?.closedAt&&Date.now()-new Date(last.closedAt).getTime()<5*60*1000){
    return{ok:true,signal:'COOLDOWN',score:plan.score,confidence:plan.confidence,scores:plan.scores,forecast24h:plan.forecast24h,plan:plan.plan,scenarios:plan.scenarios,reason:'Nghỉ 5 phút sau lệnh đóng'};
  }

  if(plan.signal==='WAIT'||!plan.order){
    return{ok:true,signal:plan.signal,score:plan.score,confidence:plan.confidence,scores:plan.scores,forecast24h:plan.forecast24h,plan:plan.plan,scenarios:plan.scenarios,reason:plan.reason};
  }
  ...
  return{ok:true,signal:plan.signal,score:plan.score,confidence:plan.confidence,scores:plan.scores,qualityGate:plan.qualityGate,entryMode:plan.entryMode,learningEligible:plan.learningEligible,forecast24h:plan.forecast24h,plan:plan.plan,scenarios:plan.scenarios,position:opened.position,reason:`Đã mở AUTO DEMO — ${plan.entryMode}`};
}
