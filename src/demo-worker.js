...
function buildAutoSignal(data, price) {
  ...
  if (signal === 'WAIT') return { signal, score, confidence, scores, reason: '5 khung chưa đạt ngưỡng mở lệnh' };
...
async function autoDemo(env) {
  ...
  return { ok: true, signal: plan.signal, score: plan.score, confidence: plan.confidence, scores: plan.scores, position: opened.position, reason: 'Đã mở AUTO DEMO' };
}
