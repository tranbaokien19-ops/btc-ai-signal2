...
<div class="card">RSI 14<div id="rsi" class="metric">--</div></div>
</div>
<div class="section"><div class="section-head"><h2>5 KHUNG THỜI GIAN AI — M5 • M15 • M30 • H1 • H4</h2><span class="muted">Mỗi khung dùng nến đúng khung thời gian</span></div><div id="tfGrid" class="tf-grid"><div class="card loading">Đang tải 5 khung...</div></div></div>
<div class="section"><div class="section-head"><h2>Đồng thuận đa khung</h2><span class="muted">Phân tích hiện tại, chưa giao dịch tiền thật</span></div><div class="consensus"><div class="card">Xu hướng chung<div id="consensusTrend" class="metric">--</div></div><div class="card">Số khung tăng<div id="bullCount" class="metric">--</div></div><div class="card">Số khung giảm<div id="bearCount" class="metric">--</div></div></div></div>
...
function drawCandles(canvas,candles){
  ...
  const live=c[c.length-1],liveY=py(live.close),liveX=px(c.length-1);x.setLineDash([4,4]);x.strokeStyle='rgba(255,255,255,.55)';x.lineWidth=1;x.beginPath();x.moveTo(left,liveY);x.lineTo(w-right,liveY);x.stroke();x.setLineDash([]);x.fillStyle='#fff';x.font='10px "Segoe UI",Arial,sans-serif';x.fillText(fmt(live.close),Math.min(w-right+3,liveX+6),Math.max(11,liveY-4));
}
...
async function tick(){try{
  ...
  renderTF(liveTF);const tm=new Date(m.updatedAt||tf.updatedAt);$('updated').textContent='Cập nhật '+tm.toLocaleTimeString('vi-VN');$('status').innerHTML='<span class="good">✓ Đã đồng bộ</span> | '+tf.source+' | <span class="live">5 EMA realtime theo đúng khung thời gian</span> | M30=2×M15, H4=4×H1 | Giá '+fmt(m.price)+' | '+tm.toLocaleTimeString('vi-VN');
...
async function loadDailyReport(){
  ...
  try{
    ...
    document.getElementById('daily-report-grid').innerHTML=
      card('Số lệnh học hôm nay',a.trades||0)+card('Thắng / Thua',(a.wins||0)+' / '+(a.losses||0))+
      card('Tỷ lệ thắng',pct(a.winRate))+card('Lãi / Lỗ',f(a.pnl))+card('R trung bình',Number(a.avgR||0).toFixed(2)+'R')+
      card('Dự báo đúng',pct(a.forecastAccuracy))+card('Kịch bản khớp',pct(a.scenarioAccuracy))+
      card('Điểm học',Number(a.avgLearningScore||0).toFixed(2));
    ...
    const hist=(d.reports||[]).map(x=>'<div style="padding:7px 0;border-bottom:1px solid #293756"><b>'+esc(x.dateVN)+'</b> | '+esc(x.trades)+' lệnh | W/L '+esc(x.wins)+'/'+esc(x.losses)+' | Tỷ lệ thắng '+pct(x.winRate)+' | Lãi/Lỗ '+f(x.pnl)+' | R TB '+Number(x.avgR||0).toFixed(2)+' | Dự báo '+pct(x.forecastAccuracy)+'</div>').join('');
    ...
    status.textContent=e.name==='AbortError'?'✕ BÁO CÁO HẰNG NGÀY QUÁ THỜI GIAN':'✕ LỖI BÁO CÁO HẰNG NGÀY'; status.style.color='#ff7181';
